...
class HierBaseline(BaseTFModel):
	...
	def train(self, global_t, sess, train_feed, update_limit=5000):
		...
		loss_names =  ["rc_loss", "rc_peplexity", "paragraph_end_loss"]
		while True:
	...
	def valid(self, name, sess, valid_feed):
		...
		while True:
			...
			rc_loss, rc_ppl, end_loss = sess.run(
				[self.avg_rc_loss,
				 self.rc_ppl, self.avg_end_loss], feed_dict)
			rc_losses.append(rc_loss)
			rc_ppls.append(rc_ppl)
			end_losses.append(end_loss)

		avg_losses = self.print_loss(name, ["rc_loss", "rc_peplexity", "end_loss"],
									 [rc_losses, rc_ppls, end_losses], "")
		return avg_losses[0]
